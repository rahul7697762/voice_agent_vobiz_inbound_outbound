...
export default function ModelSettings() {
    ...
    return (
        <div className="p-8 pb-24 max-w-4xl mx-auto relative min-h-full">
            <div className="mb-8">
                <h1 className="text-2xl font-bold text-slate-100">Models & Voice</h1>
                <p className="text-sm text-slate-400 mt-1">Select the LLM brain and TTS voice persona</p>
            </div>

            <div className="bg-[#1c2333] border border-[#2a3448] rounded-xl p-6 mb-6">
                <h2 className="text-[14px] font-semibold mb-4 pb-3 border-b border-[#2a3448] text-slate-200">Language Model (LLM)</h2>
                <div className="space-y-2 max-w-[360px]">
                    <Label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">OpenAI Model</Label>
                    <Select value={config.llm_model} onValueChange={(v: string) => setConfig({ ...config, llm_model: v })}>
                        <SelectTrigger className="bg-[#0f1117] border-[#2a3448] text-slate-200 focus:ring-[#6c63ff]">
                            <SelectValue placeholder="Select a model" />
                        </SelectTrigger>
                        <SelectContent className="bg-[#1c2333] border-[#2a3448] text-slate-200">
            ...
            <div className="bg-[#1c2333] border border-[#2a3448] rounded-xl p-6 mb-6">
                <h2 className="text-[14px] font-semibold mb-4 pb-3 border-b border-[#2a3448] text-slate-200">Voice Synthesis (Sarvam bulbul:v3)</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-[720px]">
                    <div className="space-y-2">
                        <Label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Speaker Voice</Label>
