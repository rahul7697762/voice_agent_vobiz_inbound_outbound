    ...
    return (
        <div className="p-8 pb-20 max-w-7xl mx-auto">
            <div className="mb-8">
                ...
                <p className="text-sm text-slate-500 mt-1">Real-time overview of your AI voice agent performance</p>
            </div>
            ...
            <Card className="bg-white border-slate-200 shadow-sm">
                <CardHeader className="flex flex-row items-center justify-between pb-6 border-b border-slate-200">
                    ...
                    <Button variant="ghost" size="sm" onClick={loadData} disabled={loading} className="text-slate-500 hover:text-slate-900 hover:bg-slate-100">
                        ...
                        <Table>
                            <TableHeader className="bg-slate-50/50">
                                <TableRow className="border-slate-200 hover:bg-transparent">
                                    <TableHead className="text-xs font-semibold uppercase tracking-wider text-slate-500 h-10">Date</TableHead>
                                    ...
                                    <TableHead className="text-xs font-semibold uppercase tracking-wider text-slate-500 h-10">Actions</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {loading ? (
                                    ...
                                    <TableRow><TableCell colSpan={5} className="h-24 text-center text-red-500">Could not load data — check API connection.</TableCell></TableRow>
                                ) : logs.length === 0 ? (
                                    <TableRow><TableCell colSpan={5} className="h-24 text-center text-slate-500">No calls yet. Make a test call!</TableCell></TableRow>
                                ) : (
                                    logs.slice(0, 10).map((log, i) => (
                                        <TableRow key={log.id || i} className="border-slate-200 hover:bg-slate-50/50">
                                            <TableCell className="text-slate-600">{new Date(log.created_at).toLocaleString()}</TableCell>
...
function StatCard({ title, value, desc }: { title: string, value: string | number, desc: string }) {
    return (
        <Card className="bg-white border-slate-200 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md">
